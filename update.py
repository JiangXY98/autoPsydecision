...
def extract_scores(text):
    response = client.chat.completions.create(
        model="deepseek-chat",
        messages=[
            {"role": "system", "content": "You are a psychologist and researcher. You are skilled at selecting interesting/novelty research."},
            # 更明确地要求仅输出分数行
        ...
        max_tokens=60, # 可以减少，因为只需要分数
        temperature=0.2 # 可以降低温度，减少创意性/解释性输出
    )
    ...
    research_match = re.搜索(r"Research Score:\s*(\d+)", generated_text, re.IGNORECASE)
    if research_match:
        research_score = research_match.group(1) # 提取括号匹配的数字部分

    # 查找 "Social Impact Score: <数字>"
    social_match = re.搜索(r"Social Impact Score:\s*(\d+)", generated_text, re.IGNORECASE)
    if social_match:
